...
```shell
curl --location --request PUT 'http://192.168.10.201:8500/v1/agent/service/deregister/192.168.10.96:8080'
```
...
* konga网页手动配置，比较简单，但繁琐，一旦重启kong依赖的数据库初始化，还需手动重新配置
* Kong的API接口注册服务和路由，参考`kong_init.sh`文件

注意，配置`kong+consul`需要将`Host`设为`Demo(服务名).service.consul`,而不是服务部署IP，端口为`8600`
